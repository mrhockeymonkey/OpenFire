...
class Sprite(pygame.sprite.Sprite):
    ...
    def refresh_hitbox(self, offset=None):
...
class Enemy(Sprite):
    ...
    def hit(self, damage):
        if not self.invun:
            ...
            self.action = 'hit'
            self.actions[self.action].iter()
            Damage(self.game, (self.pos.x, self.pos.y - 50), damage)
...
class Damage(Sprite):
    def __init__(self, game, pos, val):
...
class Weapon(Sprite):
    def __init__(self, game, pos):
        ...
        Sprite.__init__(self, game, pos, game.item_images['pistol']) # inherit from Sprite
...
class Bullet(Sprite):
    def __init__(self, game, pos, dir, dmg):
        ...
        self.layer = BULLET_LAYER
        Sprite.__init__(self, game, pos, game.bullet_image) # inherit from Sprite
...
class SwordStrike(Sprite):
    def __init__(self, game, pos, damage):
        ...
        self.damage = damage
        #self.rect = pygame.Rect(0, 0, 100, 100)
        self.hit_rect = pygame.Rect(0, 0, 70, 50)
        self.spawn_time = pygame.time.get_ticks() # record at what time the bullet was spawned
...
class MuzzleFlash(Sprite):
    def __init__(self, game, pos):
        self.groups = game.all_sprites
        self.layer = EFFECT_LAYER
        size = randint(20, 50)
        image = pygame.transform.scale(choice(game.gun_flashes), (size,size))
        Sprite.__init__(self, game, pos, image) # inherit from Sprite
...
class Item(Sprite):
    def __init__(self, game, pos, type):
        ...
        Sprite.__init__(self, game, pos, game.item_images[type]) # inherit from Sprite
